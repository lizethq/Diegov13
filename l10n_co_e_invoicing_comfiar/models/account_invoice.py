...
class AccountInvoice(models.Model):
    ...
    def post_comfiar(self):
        for record in self:
            if record.company_id.einvoicing_enabled and record.journal_id.is_einvoicing:
                if record.type in ("out_invoice","out_invoice_note","out_refund"):
                    ...
                    if record.type == 'out_invoice_note' and record.refund_type == 'debit':
    ...
    def _comf__get_billing_reference(self):
        billing_reference = {}
    ...
    def _comf_get_active_dian_resolution(self):
